...
data_deaths.sort(key=lambda tup: tup[0], reverse=False)
data_counts.sort(key=lambda tup: tup[0], reverse=False)
data_areas.sort(key=lambda tup: tup[0], reverse=False)
...
values.sort(key=lambda tup: tup[2], reverse=False)

# print(values)

y_pos = np.arange(len(values))

plt.bar(y_pos, list(map(lambda tup: tup[1],values)), align='center', alpha=0.5)
